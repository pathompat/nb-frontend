  ...
  <v-card :loading="loading">
    ...
    <v-data-table :loading="loading" :items="tableState" :headers="headers">
      <template v-slot:item="{ internalItem, item, isExpanded, toggleExpand }">
        <tr
          :style="{
            backgroundColor: isExpanded(internalItem) ? '#f0f0f0' : 'white',
          }"
        >
          <td @click="toggleExpand(internalItem)" class="cursor-pointer">
            <v-icon class="mr-4">{{
          ...
          <td colspan="12">
            ...
            >
              ...
              <template #item.hasPlan="{ item }">
                <v-icon v-if="item.hasPlan"> mdi-check-circle</v-icon>
              </template>
            </v-data-table>
          </td>
        </tr>
