export function useRules() {
    ...
    const passwordRule = [
        ...
        (value: string) =>
            /\d/.test(value.trim()) || 'รหัสผ่านต้องมีตัวเลขอย่างน้อย 1 ตัว',
    ]
    const emtpyRule = [(value: string) => !!value.trim() || 'กรุณากรอกข้อมูล']
    ...
    const noEmojiOrEscapeCharacterRule = [
        ...
        (v: string) =>
            !/[\\]/.test(v.trim()) || 'ไม่สามารถใช้ escape character \\ ได้',
        (v: string) => true,
    ]
    return {
        userNameRule,
